...
export default function DrawerInfo(props) {
  ...
  const selectCarRef = React.useRef()
  ...
  function approval(status) {
    if (
      selectCarRef.current.rowKeysStr == '' ||
      !selectCarRef.current.rowKeysStr
    ) {
      return message.warn('至少选择一辆车')
    }
    axios
      .get('/mobileapproval/operateapproval', {
        params: {
          flag: status,
          financeId: session.userData.userId,
          mobileApplicationId: record.id,
          vehicles: selectCarRef.current.rowKeysStr,
      ...
      <Card>
        <Descriptions title="基本信息">
          ...
          <Descriptions.Item label="审核状态">
            <Tag
              color={['yellow', 'yellow', 'green', 'red'][record.applyStatus]}
            >
              {['', '待审核', '同意', '不同意', ''][record.applyStatus]}
            </Tag>
          </Descriptions.Item>
        </Descriptions>
      </Card>
      ...
      <Card style={{ marginTop: 10 }} title="车辆列表">
        ...
        <div style={{ marginTop: 15 }}>
          {record.applyStatus == 1 ? (
            <>
              <Button type="primary" onClick={() => approval(2)}>
                同意
              </Button>{' '}
              <Button onClick={() => approval(3)}>不同意</Button>
            </>
          ) : null}
        </div>
      </Card>
